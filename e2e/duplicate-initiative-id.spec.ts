 ...
 *
 * Regression test for the initiative creation flow.
 *
 * The app generates unique initiative IDs with crypto.randomUUID(), so the
 * test asserts that three creation gestures result in three distinct bars.
 */
...
test.describe('Duplicate Initiative ID Prevention', () => {
  test.beforeEach(async ({ page }) => {
    ...
    // Clear all initiatives so the test only sees the bars it creates
